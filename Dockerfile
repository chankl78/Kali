...
LABEL org.label-schema.build-date=$BUILD_DATE \
      ...
      MAINTAINER="Chan Kuan Leang <chankl78@gmail.com>"
...
# -y means yes to all update automatically while installing update
RUN apt-get update \
    && apt-get upgrade -y \
    && apt-get clean -y \
	&& apt-get autoremove -y \
	&& apt-get -y install kali-linux-top10 exploitdb man-db dirb nikto wpscan uniscan
